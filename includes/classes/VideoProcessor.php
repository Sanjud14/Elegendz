...
class VideoProcessor
{
    ...
    private $allowedTypes = array("mp4", "flv", "webm", "mkv", "vob", "ogv", "ogg", "avi", "wmv", "mov", "mpeg", "mpg", "m4v");
    ...
    private $ffmpegPath = "ffmpeg"; // *** LOCAL! ***
    ...
    private $ffprobePath = "ffprobe"; // *** LOCAL! ***
    ...
    public function upload($videoUploadData, &$videoId)
    {

        $targetDir = "uploads/videos/";
        $videoData = $videoUploadData->videoDataArray;

        $tempFilePath = $targetDir . uniqid() . str_replace("'", "", basename($videoData["name"]));
        ...
        if (move_uploaded_file($videoData["tmp_name"], $tempFilePath)) {
            ...
            if (!$this->convertVideoToMp4($tempFilePath, $finalFilePath)) {
                echo "Upload failed. Can't convert.\n";
                return false;
            }

            if (!$this->deleteFile($tempFilePath)) {
                echo "Upload failed. Can't delete temporary file.\n";
                return false;
            }
    ...
    {
        $query = $this->con->prepare("INSERT INTO videos(title, uploadedBy, privacy, category, filePath)
                                        VALUES(:title, :uploadedBy, :privacy, :category, :filePath)");
        ...
        //$query->bindParam(":artistName", $uploadData->artistName);
